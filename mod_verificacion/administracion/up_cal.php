...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (($mes < $fecha_mes) || ($mes > $fecha_mes)) {
    header('location:../extend/alerta.php?msj=Selecciona Fecha del mes correspondiente&c=sub&p=in&t=error');
  }
  elseif (($mes2 < $fecha_mes) || ($mes2 > $fecha_mes)) {
    header('location:../extend/alerta.php?msj=Selecciona Fecha del mes correspondiente&c=sub&p=in&t=error');
  }
  elseif ($dia2 < $dia1) {
    header('location:../extend/alerta.php?msj=Fecha Incorrecta favor de revisar&c=sub&p=in&t=error');
  }
  else {
  ...
  if($up){
    ...
    $mail->FromName = "REPROGRAMACIÓN DE VERIFICACIÓN";
     ...
     if(!$mail->Send())
       ...
       header('location:../extend/alerta.php?msj=Datos Actualizados&c=sub&p=in&t=success');
       }

  }else {
    header('location:../extend/alerta.php?msj=Datos no actualizados&c=sub&p=in&t=error');
  }
  $con->close();
 }
}else {
